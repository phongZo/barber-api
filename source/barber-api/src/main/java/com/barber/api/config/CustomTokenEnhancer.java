package com.barber.api.config;

import com.barber.api.dto.AccountForTokenDto;
import com.barber.api.model.Permission;
import com.barber.api.utils.ZipUtils;
...
public class CustomTokenEnhancer implements TokenEnhancer {
    ...
    public OAuth2AccessToken enhance(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
        ...
        if(grantType == null){
            ...
            if(grantType.equals(SecurityConstant.GRANT_TYPE_CUSTOMER) ){
                ...
                additionalInfo = getAdditionalCustomerInfo(email, grantType);
            }
        }
    ...
    private Map<String, Object> getAdditionalCustomerInfo(String email, String grantType) {
        Map<String, Object> additionalInfo = new HashMap<>();
    ...
    public AccountForTokenDto getAccountByUsername(String username) {
        try {
            String query = "SELECT id, kind, username, email, full_name, is_super_admin " +
                    "FROM db_barber_account WHERE username = ? and status = 1 limit 1";
            log.debug(query);
    ...
    public AccountForTokenDto getUserByEmail(String email) {
        try {
            String query = "SELECT id, kind, username, email, full_name, is_super_admin " +
                    "FROM db_barber_account WHERE email = ? and status = 1 limit 1";
            log.debug(query);
    ...
    public List<Permission> getListPermissionByUserKind(Integer kind) {
        try{
            String query = "select * " +
                    "from db_barber_permission p " +
                    "join db_barber_permission_group g on p.id = g.permission_id " +
                    "join db_barber_account a on a.group_id = g.group_id " +
                    "where a.kind = ?";
