...
public class CustomTokenGranter extends AbstractTokenGranter {
    ...
    protected OAuth2AccessToken getAccessToken(ClientDetails client, TokenRequest tokenRequest) {
        ...
        try {
            if(SecurityConstant.GRANT_TYPE_CUSTOMER.equalsIgnoreCase(tokenRequest.getGrantType())){
                String email = tokenRequest.getRequestParameters().get("email");
                return userService.getAccessTokenForCustomer(client, tokenRequest, password, email, this.getTokenServices());
            } else{
